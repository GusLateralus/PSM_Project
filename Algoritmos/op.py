...
tiempo_limite = 3
tiempo_guardado = 7  # Tiempo para guardar datos en segundos
...
umbral_cierre = 18  # Umbral para considerar que los ojos están cerrados
umbral_movimiento = 16  # Umbral para considerar micromovimientos
...
with detector.FaceDetection(min_detection_confidence=0.5, model_selection=1) as rostros:
    with mp_holistic.Holistic(static_image_mode=False, model_complexity=1) as holistic:
        ...
        with open("registro_camara2.txt", "w") as archivo_txt:
            archivo_txt.write("Fecha,Parpadeos,MovimientoPiernas\n")
        with open('registro_camara2.txt', mode='a') as archivo_txt:
                    while cap.isOpened():
                        ...
                        if results.face_landmarks:
                            ...
                            cv2.putText(frame, 'EARD: ' + str(EARD), (0, 30), cv2.FONT_HERSHEY_SIMPLEX, 1,(255, 0, 100), 3)
                            cv2.putText(frame, 'EARI: ' + str(EARI), (0, 60), cv2.FONT_HERSHEY_SIMPLEX, 1,(255, 0, 100), 3)
######################################################################################################################################33
#ojos      
...
##################################################################################################################################
                            ...
                            if time.time() - ultima_actualizacion >= espera_actualizacion:  # Verificar cada 7 segundos
                                ...
                                if distancia_rodilla_izq < umbral_movimiento:
                                ...
                                if distancia_rodilla_der < umbral_movimiento:
...
####################################################################################################################################
                        ...
                        if time.time() - ultimo_guardado >= tiempo_guardado:
                            # Guardar datos
                            archivo_txt.write(
                                f"{time.strftime('%Y-%m-%d %H:%M:%S')},{parpadeos},{contador_movimientos}\n")
