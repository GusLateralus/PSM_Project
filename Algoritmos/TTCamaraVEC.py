...
tiempo_limite = 3
tiempo_guardado = 2  # Tiempo para guardar datos en segundos
...
with detector.FaceDetection(min_detection_confidence=0.5, model_selection=1) as rostros:
    with mp_holistic.Holistic(static_image_mode=False, model_complexity=1) as holistic:
        ...
        with open("registro_camara2.txt", "w") as archivo_txt:
            archivo_txt.write("Fecha,Parpadeos,MovimientoPiernas,MovimientoPiernas,DISTANCIA,EARD,EARI\n")
        with open('registro_camara2.txt', mode='a') as archivo_txt:
                    while cap.isOpened():
                        ...
                        if results.face_landmarks:
                            ...
                            cv2.putText(frame, 'EARD: ' + str(EARD), (0, 30), cv2.FONT_HERSHEY_SIMPLEX, 1,(255, 255, 255), 3)
                            cv2.putText(frame, 'EARI: ' + str(EARI), (0, 60), cv2.FONT_HERSHEY_SIMPLEX, 1,(255, 255, 255), 3)
######################################################################################################################################33
#ojos      
                            ...
                            cv2.putText(frame, f'Anguloder: { dy_der:.2f} cm', (0, 125), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 3)      
                            ...
                            # Mostrar la distancia física en la ventana
                            cv2.putText(frame, f'Distancia: {distancia_real_cm:.2f} cm', (0, 100), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 3)                                       
#####################################################################################################################################
########################### Supongamos que `EARI` es el indicador del estado de los ojos, actualizado en cada iteración.
                            ...
                            dy_derbase=-1.8
                            umbral_base_cierre = 20
                            factor_ajuste = 540
                            factor_dea_ajuste2=.54
                            #umbral_cierre = umbral_base_cierre + factor_ajuste * (distancia_real_cm  - distancia_base)
                            #umbral_cierre = (umbral_base_cierre + factor_ajuste / distancia_real_cm)-6
                            umbral_cierre =16
                            #umbral_cierre =18
                            umbral_movimiento=5
                            #umbral_movimiento =(umbral_movimiento + factor_dea_ajuste2 / distancia_real_cm)
                            #print(umbral_cierre)
                            #print(umbral_cierre )
                            
                            if EARI < umbral_cierre:
                                if not cerrados:
                                    cerrados = True  # Los ojos se cierran
                                    #print("Ojos cerrados")
                                else:
                                    # Detectar fluctuaciones en EARI mientras los ojos están cerrados
                                    if ultimo_valor_EARI is not None and abs(EARI - ultimo_valor_EARI) > umbral_movimiento:
                                        parpadeos_con_ojos_cerrados += 1
                                        
                                        #print("Parpadeo detectado con ojos cerrados")
                            else:
                                if cerrados:
                                    cerrados = False  # Los ojos se abren
                                    #print("Ojos abiertos")

                            # Actualizar el último valor de EARI
                            ultimo_valor_EARI = EARI
                            #print(umbral_cierre)
                            print(EARI)
                        ...
                        cv2.putText(frame, 'Parpadeos: ' + str(parpadeos_con_ojos_cerrados), (400, 30), cv2.FONT_HERSHEY_SIMPLEX, 1,
                                    (255, 255, 255), 3)
                        #print(parpadeos)
...
# Convertir las distancias de píxeles a metros
                            distancia_rodilla_izq = round(distancia_rodilla_izq * factor_conversion, 2)
                            distancia_rodilla_der = round(distancia_rodilla_der * factor_conversion, 2)
                            
##################################################################################################################################
                            umbral_movimiento2 = 0.5
                            # Verifica el estado actual en cada iteración
                            if time.time() - ultima_actualizacion >= espera_actualizacion:  # Verificar cada 7 segundos
                                # Verificar rodilla izquierda
                                if distancia_rodilla_izq > umbral_movimiento2:
                                ...
                                # Verificar rodilla derecha
                                if distancia_rodilla_der > umbral_movimiento2:
                                    if not rodilla_der_fuera:  # Detectar transición
                                        contador_movimientos += 1
                                        rodilla_der_fuera = True
                                        print("Movimiento detectado en la rodilla derecha")
                                else:
                                    rodilla_der_fuera = False  # Volvió a posición normal
...
####################################################################################################################################
                        ...
                        cv2.putText(frame, 'RI ' + str(distancia_rodilla_der), (250, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 3)
                        cv2.putText(frame, 'RD ' + str(distancia_rodilla_izq), (250, 60), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 3)
                        ...
                        if time.time() - ultimo_guardado >= tiempo_guardado:
                            # Guardar datos
                            archivo_txt.write(
                                f"{time.strftime('%Y-%m-%d %H:%M:%S')},{parpadeos_con_ojos_cerrados},{contador_movimientos},{distancia_real_cm},{EARD},{EARI}\n")
