        ...
        with open('registro_camara2.txt', mode='a') as archivo_txt:
            while cap.isOpened():
                ...
                if results.face_landmarks:
                    ...
                    ojo_der3 = results.face_landmarks.landmark[
                        387]  # Punto medio del párpado superior del ojo izquierdo
                    ojo_izq4 = results.face_landmarks.landmark[263]  # Punto medio del párpado superior del ojo derecho
                    ojo_der5 = results.face_landmarks.landmark[
                        373]  # Punto medio del párpado superior del ojo izquierdo
                ...
                if results.pose_landmarks:
                    ...
                    # Declarar el umbral y el contador al inicio
                    umbral_movimiento = 1.5  # Ejemplo: 1 metro como umbral

                    # Variables auxiliares para rastrear el estado y tiempo de las rodillas
                    rodilla_izq_supero_umbral = False
                    rodilla_der_supero_umbral = False
                    ...
                    # Verificar si ha pasado suficiente tiempo (5 segundos)
                    if tiempo_actual - ultima_actualizacion >= 2:  # 5 segundos de espera
                        # Verificar si la rodilla izquierda supera el umbral
                        if distancia_rodilla_izq < umbral_movimiento:
                            if not rodilla_izq_supero_umbral:
                                contador_movimientos += 1
                                rodilla_izq_supero_umbral = True

                        else:
                            rodilla_izq_supero_umbral = False

                        # Verificar si la rodilla derecha supera el umbral
                        if distancia_rodilla_der < umbral_movimiento:
                            if not rodilla_der_supero_umbral:
                                contador_movimientos += 1
                                rodilla_der_supero_umbral = True
                        else:
                            rodilla_der_supero_umbral = False
                ...
                cv2.putText(frame, 'RI ' + str(distancia_rodilla_der), (250, 30), cv2.FONT_HERSHEY_SIMPLEX, 1,
                            (255, 100, 0), 3)
                cv2.putText(frame, 'RD ' + str(distancia_rodilla_izq), (250, 60), cv2.FONT_HERSHEY_SIMPLEX, 1,
                            (255, 100, 0), 3)
                cv2.putText(frame, 'MP ' + str(contador_movimientos), (400, 60), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 0),
                            3)
